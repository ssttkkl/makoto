...
const InsertImageButton: React.FC = () => {
  ...
  const handleChange: UploadProps['onChange'] = (info) => {
    ...
    setFileList(info.fileList.filter((x) => x.status !== 'done'));
    if (done.length > 0) {
      console.log('upload done: ', done);

      const nodes = done.map((x) => {
        const assetId = x.response.id;
        return {
          type: 'image',
          assetId,
          children: [{ text: 'image-' + assetId }],
        };
      });
...
export default class ImagePlugin extends ElementEditorPlugin {
  key = 'image';
  toolbarItem: ToolbarItem = {
    title: '插入图片',
    renderWriteable: () => <InsertImageButton />,
  };
